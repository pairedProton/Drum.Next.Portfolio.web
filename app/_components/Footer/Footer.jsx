...
const Footer = () => {
  return (
    <div className="  footer  relative w-full h-screen bg-linear-to-r to-secondary-green from-primary-green  flex justify-center items-start ">
      ...
      <Image src={spectrum} alt='spectrum' width={1000} height={1000} className='w-full h-full object-cover' />
      </div>
...
<button type="button" className='bg-primary-green text-white font-body font-extralight text-xl shadow-md shadow-emerald-950 px-8 py-2 rounded-full border border-white' > Login  </button>
  ...
  <div className='flex flex-col justify-start items-start gap-6 w-1/2 h-auto' >
    {
      footerData.info.map((e,i)=>{
        return(
          <div key={i} className='flex flex-row justify-start items-start gap-2 font-secondary ' >
            <h3 className='text-xl font-bold shrink-0 w-24' >{e.title} : </h3>
            <p className='text-lg  font-extralight max-w-80 ' >{e.details}</p>
          </div>
        )
      })
    }
      ...
      {footerData.social.map((e,i)=>{
        return(
          <div key={i} className='flex flex-row justify-start items-start  ' >
            <Image src={e.icon} width={100} height={100} alt={e.name} className='w-10 h-10' />
          </div>
        )
      })}
    </div>
